...
void powerDistributionForceTorque(const control_t *control, float propThrusts[4], double time);
...
DLLEXPORT void webots_physics_init() {
  ...
  // Initialize CSV header for debug logging
#if DEBUG_LOGGING
  // Use "w" mode to overwrite file on each simulation run
  FILE *debug_fp = fopen(utils_GetPluginLogPath("blimp_physics_debug.csv"), "w");
  if (debug_fp) {
    fprintf(debug_fp, "time,thrust_raw,roll_raw,pitch_raw,yaw_raw,prop0,prop1,prop2,prop3\n");
    fclose(debug_fp);
  }
#endif
}
...
DLLEXPORT void webots_physics_step() {
  ...
  dVector4 propThrusts = {0};
  double *controls;
  int size;
  dBodyEnable(gRobotBody);
  ...
  double dt = controls[11];

  // Prepare Lee controller structs (assume controls[] as described above)
  control_t control = {0};
  setpoint_t setpoint = {0};
  sensorData_t sensors = {0};
  state_t state = {0};

  // Fill setpoint (desired state)
  setpoint.position.v[2] = controls[0];
  setpoint.attitude.v[2] = controls[1];
  setpoint.velocity.v[0] = controls[2];
  setpoint.attitudeRate.v[2] = controls[3]; // Yaw rate setpoint
  setpoint.velocity.v[2] = controls[4]; // Altitude velocity setpoint

  // Fill state (current state)
  state.velocity.v[0] = controls[5];
  state.velocity.v[1] = controls[6];
  state.velocity.v[2] = controls[7];
  state.position.v[2] = controls[8];
  state.attitude.v[2]  = controls[9]; // yaw
  state.omega.v[2]  = controls[10]; // yaw rate
  accumulatedTime += dt;

  controllerPid(&control, &setpoint, &sensors, &state, accumulatedTime);
  // --- Use allocation matrix to get per-motor thrusts ---
  float propThrusts_f[4] = {0};
  powerDistributionForceTorque(&control, propThrusts_f, accumulatedTime);
  for (int i = 0; i < 4; ++i) propThrusts[i] = (double)propThrusts_f[i];
...
void powerDistributionForceTorque(const control_t *control, float propThrusts[4], double time_sim) {

    // Compute the individual motor thrusts: propThrusts = A_inv * wrench
    for (int i = 0; i < 4; ++i) {
        propThrusts[i] = 0.0f;
    }

    propThrusts[0] = control->thrust;
    propThrusts[1] = control->thrust;
    propThrusts[2] = control->pitch - control->yaw;
    propThrusts[3] = control->pitch + control->yaw;


    for (int i = 0; i < 4; ++i) {
      propThrusts[i] = propThrusts[i] / 65535;
      propThrusts[i] = 0.09142681 * propThrusts[i] * propThrusts[i] + 0.067673604 * propThrusts[i];
      if(propThrusts[i] < 0) {
          propThrusts[i] = 0;
      }
    }

    // Debug log in CSV format: time,thrust_raw,roll_raw,pitch_raw,yaw_raw,prop0,prop1,prop2,prop3
    DEBUG_LOG("blimp_physics_debug.csv", "%.3f,%.4f,%.4f,%.4f,%.4f,%.6f,%.6f,%.6f,%.6f\n",
              time_sim, control->thrust, control->roll, control->pitch, control->yaw,
              propThrusts[0], propThrusts[1], propThrusts[2], propThrusts[3]);
}
