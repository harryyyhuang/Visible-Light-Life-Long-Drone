...
void compRestoringWrench(const dReal *inGenPosA, dReal *genForceAB) {
  ...
  // Gravity
  genForceAB[0] += (net_weight) * g * dSin(inGenPosA[4]);
  genForceAB[1] += (net_weight) * g * dCos(inGenPosA[4]) * dSin(inGenPosA[3]);

  genForceAB[2] += -(net_weight) * g * dCos(inGenPosA[4]) * dCos(inGenPosA[3]);
